...
- **Hidden backlink growth engine adopted (Phase 2+)** — Content pipeline optimized page includes `<link rel="ai-analytics" href="crawlready.app/score/{domain}">` for free-tier users. Invisible to humans, discoverable by crawlers. Injected during optimized page generation — not via customer middleware or script tag. Paid tiers can remove it. Opt-in visible badge available but never forced. Creates SEO compounding loop. See `docs/architecture/analytics-infrastructure.md` §Hidden Backlink Architecture.
